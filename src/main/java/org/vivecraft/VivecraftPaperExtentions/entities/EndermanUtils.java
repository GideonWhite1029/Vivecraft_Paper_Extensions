...
public class EndermanUtils {
	...
	{
		ItemStack itemstack = pPlayer.getInventory().equipment.get(EquipmentSlot.HEAD);
		...
		else
		{
			...
			//VPE MODIFICATION
			boolean vr = pPlayer instanceof Player && VPE.isVive((org.bukkit.entity.Player) pPlayer.getBukkitEntity());
			VivePlayer vp = null;
			Vec3 hmdpos = null;
			double f=0.025;
			if(vr){
				vp = VPE.vivePlayers.get(pPlayer.getBukkitEntity().getUniqueId());
				vec3 = vp.getHMDDir();
